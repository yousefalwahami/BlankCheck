 ...
 * shell-order commitments, sealed envelopes, accusations, verdicts, chips, buy-ins and the pot.
...
#define BC_MAX_BUY_IN_CHIPS (10U)
...
/* Instructions */
#define BC_IX_CREATE_TABLE  (0U) /* host: u64 game_id, u8 n, u8 buy_in_chips, u8 rounds, u8 wallets[n][32], u32 proof_sz, proof */
...
#define BC_IX_BUY_IN        (8U) /* host: u8 round, u8 seat, u8 payment[64] (signature of the $12 token transfer) */
#define BC_IX_END_ROUND     (9U) /* host: u8 round */
...
#define BC_ERR_TURN      (0x2009UL) /* not your turn / someone has no chips */
...
#define BC_ERR_CHIPS     (0x2013UL) /* buying in while you still have chips */
...
#define BC_EVT_BUY_IN          (11U)
#define BC_EVT_POT_AWARDED     (12U)
...
typedef struct __attribute__((packed)) {
  uchar  magic[4]; /* "BCK2" (chips edition) */
  uchar  host[32]; /* game server pubkey (fee payer of every host instruction) */
  ulong  game_id;
  uchar  status;
  uchar  num_seats;
  uchar  buy_in_chips;    /* chips per $12 buy-in */
  uchar  rounds_total;    /* the game cashes out after this many rounds */
  uchar  round;           /* BC_NONE before the first round */
  uchar  round_ended;     /* 1 once END_ROUND has paid out the pot */
  ...
  uchar  winner;                   /* BC_NONE until finished: the biggest profit */
  ushort pot;                      /* chips knocked off by live shells; the leader takes it at END_ROUND */
  ushort chips[BC_MAX_SEATS];      /* 0 = broke: skipped and untargetable until BUY_IN */
  uchar  buy_ins[BC_MAX_SEATS];
...
_Static_assert(sizeof(bc_table_t) == 74850UL, "table layout drifted from packages/shared/src/table.ts");
_Static_assert(offsetof(bc_table_t, game_id) == 36UL, "layout");
_Static_assert(offsetof(bc_table_t, pot) == 58UL, "layout");
_Static_assert(offsetof(bc_table_t, chips) == 60UL, "layout");
_Static_assert(offsetof(bc_table_t, buy_ins) == 72UL, "layout");
_Static_assert(offsetof(bc_table_t, seat_wallet) == 90UL, "layout");
_Static_assert(offsetof(bc_table_t, shells_commit) == 282UL, "layout");
_Static_assert(offsetof(bc_table_t, window_count) == 1098UL, "layout");
_Static_assert(offsetof(bc_table_t, env) == 1122UL, "layout");
