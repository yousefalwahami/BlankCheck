...
let acct: Awaited<ReturnType<typeof thru.accounts.get>> | undefined;
for (let i = 0; i < 12; i++) {
  try {
    acct = await thru.accounts.get(result.tableAddress);
    break;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    if (i === 11 || !/seq mismatch|unavailable/i.test(msg)) throw err;
    const wait = 400 * (i + 1);
    console.log(`  (table account read retry ${i + 1}/12 after ${wait}ms: ${msg})`);
    await new Promise((r) => setTimeout(r, wait));
  }
}
if (!acct) throw new Error("could not read Table account");
